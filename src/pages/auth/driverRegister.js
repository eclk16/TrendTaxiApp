...
import {Alert, BackHandler, Linking, ScrollView, TextInput} from 'react-native';
...
export default function DriverRegister({isDriver,setIsDriver}){
    ...
    const [driverImage,setDriverImage] = React.useState({uri:''});
    const [uploadDriverImage,setUploadDriverImage] = React.useState('');

    const [driverLicenseImage,setDriverLicenseImage] = React.useState({uri:''});
    const [uploadDriverLicenseImage,setUploadDriverLicenseImage] = React.useState('');

    const [driverRuhsatImage,setDriverRuhsatImage] = React.useState({uri:''});
    const [uploadDriverRuhsatImage,setUploadDriverRuhsatImage] = React.useState('');

    const [driverCarImage,setDriverCarImage] = React.useState({uri:''});
        ...
        <SafeAreaView style={pt.sav}>
            ...
            <TouchableOpacity style={[tw`flex-row items-center px-4 py-2`]} onPress={() => {
                ...
                <View style={[tw`flex-row justify-center items-center `]}>
                    ...
                    <MaterialCommunityIcons name={step == 2 ? 'car' : 'circle-small'} size={40} color={stil('text',data.app.theme).color}/>
                    <MaterialCommunityIcons name={step == 3 ? 'file-document' : 'circle-small'} size={40} color={stil('text',data.app.theme).color}/>
                </View>
                <View>
                    {step == 1 &&
                        <>
                            ...
                            <View style={[tw`m-4 rounded-md`]}>
                                ...
                                <View style={[tw`rounded-md p-2 mt-2 flex-row items-center justify-start`,stil('bg2',data.app.theme)]}>
                                    ...
                                    <View style={[tw`ml-4`]}>
                                        ...
                                        >
                                        <Text style={[tw` w-3/4`,stil('text',data.app.theme)]}>{l[data.app.lang].dciu}</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                                <View style={[tw`rounded-md p-2 mt-2 flex-row items-center justify-start`,stil('bg2',data.app.theme)]}>
