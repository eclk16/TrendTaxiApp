...
function Driver() {
    ...
    const uygunum = (lat, lon) => {
        ...
        axios.get('https://trendtaxi.uz/event?prc=available&driver_id=' + data.auth.userId + '&location=' + current);
    };
    ...
    const mapConfiguration = () => {
        ...
        setSource({uri: 'https://trendtaxi.uz/maps?me=' + current + '&token=' + data.auth.userToken + param});
    };
    ...
    return (
        <>
            <View style={[tw` w-full`, {height: topHeight}]}>
                ...
                <View style={[tw`flex-row items-center justify-center  px-2  rounded-t-md`, {
                    position: 'absolute',
                    bottom: 10,
                    left: 0,
                    right: 0,
                    ...
                    <TouchableOpacity
                        style={[tw` flex-row items-center justify-center py-3 w-full rounded-md w-full ${isActive ? 'bg-green-700' : 'bg-gray-700'}`]}
                        onPress={() => {
                            setIsActive(!isActive);
                            getCurrentLocation();
                        }}
                    >
                        <Text style={[tw`font-medium ml-2 text-white`]}>{l[data.app.lang].status}
                            : {isActive ? l[data.app.lang].active : l[data.app.lang].inactive}</Text>
                    </TouchableOpacity>
                </View>
            </View>
            <View
                    ...
                    {reports?.day &&
                        <View style={[tw` w-1/1`]}>
                            <View style={[tw`p-2 flex`]}>
                                <View
                                    style={[tw`flex-row justify-between items-center border-b-2 p-2`, {borderColor: stil('bg2', data.app.theme).backgroundColor}]}>
                                    <Text style={[stil('text', data.app.theme), tw`font-medium`]}>{l[data.app.lang].count}</Text>
                                    <Text style={[stil('text', data.app.theme), tw``]}>{reports.day.count}</Text>
                                </View>
                                <View
                                    style={[tw`flex-row justify-between items-center border-b-2 p-2`, {borderColor: stil('bg2', data.app.theme).backgroundColor}]}>
                                    <Text style={[stil('text', data.app.theme), tw`font-medium`]}>{l[data.app.lang].kilometer}</Text>
                                    <Text style={[stil('text', data.app.theme), tw``]}>{reports.day.km}</Text>
                                </View>
                                <View
                                    style={[tw`flex-row justify-between items-center border-b-2 p-2`, {borderColor: stil('bg2', data.app.theme).backgroundColor}]}>
                                    <Text style={[stil('text', data.app.theme), tw`font-medium`]}>{l[data.app.lang].time}</Text>
                                    <Text style={[stil('text', data.app.theme), tw``]}>{reports.day.time}</Text>
                                </View>
                                <View
                                    style={[tw`flex-row justify-between items-center border-b-2 p-2`, {borderColor: stil('bg2', data.app.theme).backgroundColor}]}>
                                    <Text style={[stil('text', data.app.theme), tw`font-medium`]}>{l[data.app.lang].price}</Text>
                                    <Text style={[stil('text', data.app.theme), tw``]}>{reports.day.price}</Text>
                                </View>
                                <View style={[tw`flex-row justify-between items-center p-2`]}>
                                    <Text style={[stil('text', data.app.theme), tw`font-medium`]}>{l[data.app.lang].fee}</Text>
            ...
            >
                ...
                <View style={[tw` flex items-center justify-end`,{position:'absolute',bottom:0}]}>
                    <View
                        style={[tw` w-full flex items-end justify-end rounded-md p-4 px-6 pb-8`, stil('bg', data.app.theme)]}>
                        <View style={[tw`mb-4  w-full`]}>
                            <View
                                style={[tw` rounded-md mb-1`, stil('bg2', data.app.theme), stil('text', data.app.theme)]}>
                                <Text style={[tw`font-semibold text-center p-2`, stil('text', data.app.theme)]}>{gelenTrip.yolcu.first_name} {gelenTrip.yolcu.last_name}</Text>
                            </View>
                            <View style={[tw`flex`]}>
                                <View style={[tw`flex-row justify-between items-center my-2`]}>
                                    <View
                                        style={[tw`w-1/3 rounded-tl-md rounded-bl-md mb-1`, stil('bg2', data.app.theme), stil('text', data.app.theme)]}>
                                        <Text style={[stil('text', data.app.theme), tw`font-bold text-center p-2`]}>{gelenTrip.est_time}
                                            </Text>
                                    </View>
                                    <View
                                        style={[tw`w-1/3  mb-1`, stil('bg2', data.app.theme), stil('text', data.app.theme)]}>
                                        <Text style={[stil('text', data.app.theme), tw`font-bold text-center p-2`]}>{gelenTrip.est_distance}
                                            </Text>
                                    </View>
                                    <View
                                        style={[tw`w-1/3 rounded-tr-md rounded-br-md mb-1`, stil('bg2', data.app.theme), stil('text', data.app.theme)]}>
                                        <Text style={[stil('text', data.app.theme), tw`font-bold text-center p-2`]}>{gelenTrip.est_price} sum</Text>
                                    </View>
                                </View>
                        ...
                        <View style={[tw`flex items-center justify-between w-full`]}>
                            ...
                            >
                                <View style={[tw` h-[100%] w-[${100 - (timeoutSn * 10)}%] rounded-md bg-green-900`, {
                                    left: 0,
                                    position: 'absolute',
                                    zIndex: 999,
                                }]}></View>
                                <MaterialCommunityIcons style={{zIndex: 9999}} name="check" size={20}
                                                        color="white"/>
                                <Text
                                    style={[tw`font-semibold ml-2 py-4 text-white`, {zIndex: 9999}]}>{l[data.app.lang].check}
                                    ({timeoutSn} sn)</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
