...
function TripDetail() {
    ...
    const url = {
        ...
        trip:'https://trendtaxi.uz/api/getTrip',
    };
    ...
    const getTrip = (id,userToken) => {
        ...
        .then(response => {
            if(!response.data.data.hata) {
                ...
                setTripDriver(response.data.data.sofor.first_name + ' ' + response.data.data.sofor.last_name);
                setTripDriverImage(response.data.data.sofor_image);
                setTripPassengerImage(response.data.data.yolcu_image);
                setTripPassenger(response.data.data.yolcu.first_name + ' ' + response.data.data.yolcu.last_name);
                setTripCar(response.data.data.sofor.araba.plaka);
                setTripCarImage('https://trendtaxi.uz'+response.data.data.sofor.araba.image);
    ...
    return (
        <ScrollView style={[stil('bg',data.app.theme),tw`p-4 flex-1`]}>
            ...
            {userType === 'driver' ?
                <>
					<Text style={[stil('text',data.app.theme),tw`mt-2 `]}>{l[data.app.lang].passenger}</Text>
					<View style={[tw`rounded-md px-4 py-2 my-2 flex items-start justify-start`,stil('bg2',data.app.theme)]}>
						<View style={[tw`flex w-full mb-2`]}>
							<Image source={{uri:tripPassengerImage}} style={[tw`h-24 w-24 rounded-2 mr-2 mb-2`]}/>
            ...
            {userType === 'passenger' ?
                <>
					...
					<View style={[tw`rounded-md p-4 my-2 flex items-start justify-start`,stil('bg2',data.app.theme)]}>
